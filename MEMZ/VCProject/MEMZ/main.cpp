...
void main() {
	...
	if (argc > 1) {
		...
		if (MessageBoxA(NULL, "这是个病毒。。。算了不翻译了", "MEMZ - harmless by LWYS", MB_YESNO | MB_ICONWARNING) != IDYES ||
MessageBoxA(NULL, "啥？", "MEMZ", MB_YESNO | MB_ICONWARNING) != IDYES|| MessageBoxA(NULL, "你真的想进去？可我还没有码完呢！", "MEMZ", MB_YESNO | MB_ICONWARNING) != IDYES) {
			ExitProcess(0);
		}
